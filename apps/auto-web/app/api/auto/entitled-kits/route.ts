// /api/auto/entitled-kits — the user's PROTECTED entitled Market kits, for the
// "run on Auto" picker (BROWSER / cookie auth).
//
...
// always empty (fail closed) so the picker is absent and the rest of Auto
// (local/free kits) still works. A service failure also degrades to an empty
// list rather than failing the page load.
...
export async function GET() {
  ...
  // call out, return an empty list so the UI hides the picker.
  if (!isMarketEnabled()) {
  ...
  try {
    ...
    // A listing failure must not hard-fail the Auto page load. Degrade to an
    // empty list (the picker hides) and log for observability instead of 500.
    console.error("[auto] listEntitledKitsViaService failed", error);
    return Response.json({ kits: [] }, { status: 200 });
  }
}
