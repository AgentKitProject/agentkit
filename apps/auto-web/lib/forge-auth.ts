...
export async function requireForgeUser(request: Request): Promise<ForgeAuthenticatedUser> {
  // Device-auth (WorkOS BEARER) is WorkOS-bound and unused on self-hosted (OIDC)
  // instances. Under AUTH_PROVIDER=oidc the whole /api/forge/auto/* device-bearer
  // surface is inert: return a clean 501 rather than attempting a WorkOS JWKS
  // verification that cannot succeed (the self-host browser/cookie path drives
  // Auto runs instead). Hosted (workos) is unchanged. Mirrors agentkitforge-web.
