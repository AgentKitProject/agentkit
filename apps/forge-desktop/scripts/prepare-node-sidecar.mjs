// Stages the standalone Node "sidecar" binary the packaged Tauri app ships and
// runs as `externalBin` ("binaries/node"). This is a *packaging* concern, not a
// JS build step: it needs a statically-linkable Node binary (a Homebrew/system
// Node is dynamically linked against @rpath/libnode and cannot be copied as a
// self-contained sidecar). It therefore runs from `build:tauri` (release CI),
// not from `build`/`build:backend`, so `turbo run build` works without a
// release-grade Node binary or the Rust toolchain.
//
// Provide a suitable binary via AGENTKITFORGE_NODE_SIDECAR=/abs/path/to/node
// (e.g. an unofficial statically-linked Node build) when running locally.
