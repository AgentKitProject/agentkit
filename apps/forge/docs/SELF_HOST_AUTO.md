...
```
                 ┌────────────────────────────────────────────┐
  user / API ──▶ │  agentkitforge (Next.js, selfhost)      │
...
```sh
helm upgrade --install agentkitforge charts/agentkitforge \
  --namespace agentkitforge --create-namespace \
  --values charts/agentkitforge/values-k3s.yaml \
