...
import { AdminApiError, getSubmissionById } from "@/lib/admin-api";
import { ForbiddenError, requireUserForApi, UnauthorizedError } from "@/lib/auth";
import { isOwnSubmission, sanitizeUserSubmissionDetail } from "@/lib/user-submissions";
...
export async function GET(_request: Request, { params }: RouteContext) {
  try {
    ...
    return NextResponse.json({ item: sanitizeUserSubmissionDetail(item) });
  } catch (error) {
    return userSubmissionErrorResponse(error, "Submission detail request failed.");
  }
}
