...
function rowToKit(row: any): KitRecord {
  return {
    ...
    // Tier-2 pricing/license columns are WRITTEN only by the commercial overlay
    // (@agentkit-commercial/market-core), but the public read path still surfaces
    // them so the public catalog serializer + the paid-download gate work when
    // the commercial schema is present. On the free schema these columns are
    // absent (SELECT * omits them) and read back as undefined ⇒ free behavior.
    ...
    perRunRoyaltyCents: num(row.per_run_royalty_cents),
...
function rowToOrganization(row: any): Organization {
  return {
    ...
    // Stripe Connect seller-payout fields are WRITTEN only by the commercial
    // overlay, but the public read path surfaces them so the commercial payout-
    // status route can read them back. Absent on the free schema ⇒ undefined.
...
async function upsertKit(pool: PgQueryable, k: KitRecord): Promise<void> {
  // Tier-2 pricing/license columns are owned by the commercial schema/adapter
  // and are NOT written here; a re-publish leaves any commercial-set pricing
  // untouched (those columns are absent from the free DDL). The KitRecord type
  // still carries the optional fields, but the free path never persists them.
  await pool.query(
    `INSERT INTO kits (
