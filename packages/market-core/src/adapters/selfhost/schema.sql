...
CREATE TABLE IF NOT EXISTS kits (
  ...
  owner_org_id        text,
  visibility          text,
  -- NOTE: Tier-2 paid/licensed-kit columns (pricing, price_*, currency,
  -- interval, trial_days, downloadable, license_*) live in the COMMERCIAL
  -- schema (@agentkit-commercial/market-core) and are added to this table only
  -- when the commercial overlay runs. The free build neither defines nor uses them.
...
CREATE TABLE IF NOT EXISTS organizations (
  ...
  -- NOTE: Stripe Connect seller-payout columns (stripe_account_id,
  -- charges_enabled, payouts_enabled, payout_onboarded_at) live in the
  -- COMMERCIAL schema (@agentkit-commercial/market-core); the free build omits them.
  created_at             text NOT NULL,
  updated_at             text NOT NULL
);
