...
/**
 * Commercial-only: persists Tier-2 pricing/license metadata on a kit. Split off
 * the public `AdminRepository` so the open-source/free build carries no pricing
 * write path. The commercial package (`@agentkit-commercial/market-core`)
 * implements this against DynamoDB / Postgres; the public adapters do not.
 *
 * Caller enforces role gates + validation (paid requires priceCents>0 +
 * priceModel; subscription requires interval); the repository persists the
 * resolved fields and returns the kit.
 */
...
/**
 * Commercial-only: Stripe Connect seller-payout persistence on an org. Split off
 * the public `OrgRepository` so the free build carries no Stripe coupling. Core
 * never calls Stripe; market-app resolves these from the Stripe API (account
 * create / account.updated) and passes them in. payoutOnboardedAt is stamped by
 * the caller the first time payouts become enabled. The commercial package
 * implements this; the public adapters do not.
 */
export interface CommercialPayoutRepository {
