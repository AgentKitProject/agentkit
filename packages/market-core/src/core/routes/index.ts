...
export async function routeRequest(request: CoreRequest, deps: RouterDeps): Promise<CoreResponse> {
  ...
  try {
    ...
    if (request.resource.startsWith('/admin/') || request.resource.startsWith('/users/')) {
      ...
      // Paid/licensed-kit pricing, entitlements, watermarked licensed-package,
      // and Stripe Connect seller payouts live in @agentkit-commercial/market-core.
      // When that package is injected as deps.commercial it owns these routes;
      // otherwise they fall through to the free 404 below.
      if (deps.commercial) {
        const commercialResponse = await deps.commercial.handle(request, deps);
...
async function createKitDownloadUrl(
  ...
  // served via the public presigned download — buyers go through the
  // entitlement-gated licensed-package route, which injects a per-buyer
  // watermark. The gate is INJECTED by the commercial router; absent it (the
  // free build) every kit is downloadable, so free downloads always work.
  if (commercial?.isDownloadGated?.(kit) ?? false) {
