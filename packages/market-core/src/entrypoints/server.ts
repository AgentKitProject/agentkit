...
export async function startServer(): Promise<StartServerResult> {
  ...
  // OPTIONAL commercial overlay (Tier-2 paid kits + Stripe payouts). Present only
  // when @agentkit-commercial/market-core is installed; absent on the free build.
  const commercialLoaded = await loadCommercial(pool as unknown as PgPool);
  ...
  try {
    ...
    // Apply the commercial schema (entitlements table + Tier-2/Stripe columns)
    // under the same lock when the overlay is present.
    if (commercialLoaded) {
      await schemaClient.query(commercialLoaded.schemaSql);
    }
  } finally {
...
interface ServerDeps {
  ...
  /** Optional commercial router (Tier-2 paid kits + Stripe payouts). */
  commercial?: CommercialRouter;
  /** Commercial route templates merged into the request matcher. */
...
/**
 * OPTIONALLY loads `@agentkit-commercial/market-core` and builds the Postgres-
 * backed commercial router + its DDL. Absent on the free/open-source build (the
 * dynamic import fails → returns undefined), in which case the server runs the
 * free path only.
 */
async function loadCommercial(pool: PgPool): Promise<LoadedCommercial | undefined> {
  try {
    const mod = await import('@agentkit-commercial/market-core');
    return {
      router: mod.createPostgresCommercialRouter(pool),
      routes: mod.COMMERCIAL_ROUTES,
      schemaSql: mod.COMMERCIAL_SCHEMA_SQL,
    };
  } catch {
    return undefined;
  }
...
async function handleRequest(req: IncomingMessage, res: ServerResponse, deps: ServerDeps): Promise<void> {
  try {
    const coreRequest = await toCoreRequest(req, deps.extraRoutes ?? []);
...
async function toCoreRequest(req: IncomingMessage, extraRoutes: RoutePattern[] = []): Promise<CoreRequest> {
  ...
  // parameters so the router's resource dispatch works unchanged. Commercial
  // route templates (when the overlay is present) are merged into the matcher.
  const { resource, pathParameters } = matchRoute(method, url.pathname, extraRoutes);
